...
@onready var limit_min: Marker2D = $LimitMin
# The maximum position of the camera's lower right corner
@onready var limit_max: Marker2D = $LimitMax
...
func _process(delta: float) -> void:
	...
	if Connections.isDedicatedServer():
		return
	var player: CharacterBody2D = Characters.getMyCharacterNode()
	if player == null:
		return
...
func _track_player(player: CharacterBody2D, delta: float) -> void:
	"""
	Tracks a player while staying within the limits specified by child LimitMin 
	and LimitMax Marker2D nodes.
	"""
	# Interpolate the camera's offset to the main player's position
	offset = offset.lerp(player.global_position, delta * smoothing)
	# Make sure camera's size do not go past set limits
