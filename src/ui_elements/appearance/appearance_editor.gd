...
func _generateTabs() -> void:
	...
	var colors = load(colorScene).instantiate()
	colors.connect("setColor", Callable(self, "_on_color_selected"))
...
func _addChildTab(partName: String) -> void:
	...
	assert(not files.is_empty()) #,"Empty file list")
...
func _createChildTab(resource: String) -> ItemList:
	...
	child.connect("item_selected", Callable(self, "_on_item_selected")) ## Connect item selection signal
...
func _populateChildTab(files: Dictionary, resource: String, child: ItemList) -> void:
	...
	assert(not files[resource].is_empty()) #,"Empty resource list")
